...
func seriesAsPercent(arg, total []*types.MetricData) []*types.MetricData {
	if len(total) == 0 {
		...
		} else if len(arg)%len(total) == 0 {
			// asPercent(seriesList, totalSeriesList) for series with aligned length
			// len(seriesList) > len(totalSeriesList) and len(seriesList) % len(totalSeriesList) = 0
			for n, a := range arg {
				nTotal := n / len(total)
				for i := range a.Values {
					t := total[nTotal].Values[i]
					if math.IsNaN(a.Values[i]) || math.IsNaN(t) || t == 0 {
						a.Values[i] = math.NaN()
					} else {
						a.Values[i] *= 100 / t
				...
				a.Name = "asPercent(" + a.Name + "," + total[nTotal].Name + ")"
			}
		} else {
		...
		if argGroup, exists := argGroups[key]; exists {
			if totalGroup, exist := totalGroups[key]; exist {
				if len(argGroup) <= len(totalGroup) {
					...
					sort.Sort(helper.ByName(arg[start:end]))
				} else if len(argGroup)%len(totalGroup) == 0 {
					// asPercent(seriesList, totalSeriesList, *nodes) for series with aligned length
					// len(seriesGroupList) > len(totalSeriesGroupList) and len(seriesGroupList) % len(totalSeriesGroupList) = 0

					start := len(arg)
					for n, a := range argGroup {
						nTotal := n / len(totalGroup)
						for i := range a.Values {
							t := totalGroup[nTotal].Values[i]
							if math.IsNaN(a.Values[i]) || math.IsNaN(t) || t == 0 {
								a.Values[i] = math.NaN()
							} else {
								a.Values[i] *= 100 / t
						...
						a.Name = "asPercent(" + a.Name + "," + totalGroup[nTotal].Name + ")"
						arg = append(arg, a)
					}
					end := len(arg)
					sort.Sort(helper.ByName(arg[start:end]))
